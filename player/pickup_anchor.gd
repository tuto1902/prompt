...
func _on_prompt_response_collected(_terminal_id: Enums.TERMINALS, _save_game: bool) -> void:
	var current_pickup = get_tree().get_first_node_in_group("Pickup")
	if current_pickup:
		return
	var pickup: Pickup = load(GameManager.pickup_scene).instantiate() as Pickup
